...
Project note: this repository owns independent selected-Test262 evidence. Keep Test262 outside git, preserve explicit eligibility/exclusion counts, run original and transformed programs under the same pinned runtime, and never count source/runtime failures as Minify++ failures. Confirmed defects must be reduced into the product regression suite.
